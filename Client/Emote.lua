...
local khyraweena = false
local babykopen = false
    ...
    if khyraweena then 
     ...
     -- print("khyraweena")
      ...
      elseif IsControlPressed(0, 202) then 
        ...
        khyraweena = false
        babykopen = false
      elseif khyraweena and IsControlPressed(0, 170) then 
        TriggerEvent("m:kfalse")
        khyraweena = false
        babykopen = false
      end 
...
RegisterCommand('emotemenuf3', function()
  if not babykopen then
    babykopen = true
  khyraweena = true
  --plyState.belle = true
  TriggerEvent("m:k")
...
-- RegisterCommand('-showbills', function()
    -- showbills = false
-- end, false)
RegisterKeyMapping('emotemenuf3', 'Emotes Menu', 'keyboard', 'F3')
...
local bindKeys = { 159, 161, 162, 163, }

RegisterCommand('bindemote', function(source, args)
  local bind = tonumber(args[1])
  local emote = tostring(args[2])

  if bind == nil or bind <= 0 or bind > 5 or emote == nil then
    return
  end

  if DP.Emotes[emote] == nil and DP.Dances[emote] == nil and DP.PropEmotes[emote] == nil then
    EmoteChatMessage("'"..emote.."' "..Config.Languages[lang]['notvalidemote'].."")
    return    
  end

  bindedEmotes[bind] = emote

  TriggerServerEvent('pyrp_emotes:UpdateKeyBinds', bindedEmotes)
end)

RegisterNetEvent('pyrp_emotes:SendSavedEmotes')
AddEventHandler('pyrp_emotes:SendSavedEmotes', function(data)
  bindedEmotes = data
end)

RegisterNetEvent('pyrp_emotes:PlayBindedEmote')
AddEventHandler('pyrp_emotes:PlayBindedEmote', function(key)

  if bindedEmotes[key] == nil then
    return
  end
  ...
  ExecuteCommand("e " .. bindedEmotes[key])
end)
...
function EmoteChatMessage(args)
  ...
  --  TriggerEvent("chatMessage", "^5Help^0", {0,0,0}, string.format(""))
	TriggerEvent('chat:addMessage', {
		template = '<div class="chat-message">^1[HELP] ^0{0}</div>',
		args = { string.format("") }
	})
  else
...
AddEventHandler('pyrp_polyzone:raycast:update', function(entity, entityType, entityCoords)
  if entity == nil or entityType ~= 1 then 
    FacingPed = nil
    return
  end
...
function OnEmotePlay(EmoteName)
  ...
  if ename == "Slap" and FacingPed ~= nil and not IsPedInAnyVehicle(FacingPed, true) and (GetGameTimer() - LastSlap) > 5000 then
    local playerPed = PlayerPedId()
    local pedCoords = GetEntityCoords(playerPed)
    if #(pedCoords - GetEntityCoords(FacingPed)) < 3.0 then
      TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 3.0, 'slap', 0.5)
      if IsPedAPlayer(FacingPed) then
        if not IsPlayerDead(NetworkGetPlayerIndexFromPed(FacingPed)) then
          TriggerServerEvent('dpemotes:server:SlapPlayer', GetPlayerServerId(NetworkGetPlayerIndexFromPed(FacingPed)))
        end
      else
        Citizen.Wait(500)
        SetPedToRagdoll(FacingPed, 1000, 1000, 0, 0, 0, 0)     

        SetPedRelationshipGroupHash(FacingPed, GetHashKey("_ATTACK_PLAYER"))
        SetPedCombatAttributes(FacingPed, 5, true)
        SetPedCombatAttributes(FacingPed, 46, true)
        SetPedFleeAttributes(FacingPed, 2, true)
        TaskCombatPed(FacingPed, playerPed, 0, 16)
      end
...
local LastSlapped = 0
RegisterNetEvent('dpemotes:client:PlaySlapped')
AddEventHandler('dpemotes:client:PlaySlapped', function()
  local playerPed = PlayerPedId()

  if IsPlayerDead(PlayerId()) or (GetGameTimer() - LastSlapped) < 5000 then return end
  LastSlapped = GetGameTimer()

  if math.random(1, 100) < 40 then
    LoadAnim("random@mugging4")
    TaskPlayAnim(PlayerPedId(), "random@mugging4", "flee_backward_loop_shopkeeper", 2.0, 2.0, 5000, 1, 0, 0, 0, 0)
  else
